...
# 锚点框的初步筛选：根据真实框中心点、目标框来判断
    ...
    # x_shifts,y_shifts: 每个方格的xy偏移量,size:[1,8400], 80*80+40*40+20*20
    # total_num_anchors: 所有anchor数，num_gt: 某幅图像的目标框个数
        ...
        # 方格中心点与真实框的左、右、上、下的距离，size:[num_gt,8400],每个gt目标8400个anchor
        b_l = x_centers_per_image - gt_bboxes_per_image_l
        b_r = gt_bboxes_per_image_r - x_centers_per_image
        b_t = y_centers_per_image - gt_bboxes_per_image_t
        b_b = gt_bboxes_per_image_b - y_centers_per_image
        bbox_deltas = torch.stack([b_l, b_t, b_r, b_b], 2) # bbox_deltas size:[num_gt,8400,4] 在第三维度叠加坐标向量生成bbox向量
        ...
        # 上面的正样本选取方法是基于gt框的 即在框内的anchor都算正样本 下面的是放宽要求的，旨在增加正样本数量 选取落在以gt框中心5倍stride的正方形作为正样本选区范围
        # in fixed center
        ...
        is_in_centers = center_deltas.min(dim=-1).values > 0.0  #找出方格中心点落在5倍边长正方形中的anchor,size: [num_gt,8400] -->输出在的个数
        is_in_centers_all = is_in_centers.sum(dim=0) > 0        #8400个anchor是否存在目标预测(in center box[5 times stride])  -->输出在的个数
    ...
    def dynamic_k_matching(self, cost, pair_wise_ious, gt_classes, num_gt, fg_mask):
        ...
        n_candidate_k = min(10, ious_in_boxes_matrix.size(1)) # 选取bbox与gt产生的IoU矩阵前十个与矩阵每一行的元素数量取小 https://blog.csdn.net/YMilton/article/details/120699251 看图 若生成的IoU数量小于10则取小的
        topk_ious, _ = torch.topk(ious_in_boxes_matrix, n_candidate_k, dim=1) # dim=0为列 dim=1为行
        dynamic_ks = torch.clamp(topk_ious.sum(1).int(), min=1) # 统计每个目标分配的候选框数量 按行相加所有topk的iou后取整后限制范围下限为1，保证至少有一个以上的正样本 torch.clamp() 限制张量内元素大小上下限
        dynamic_ks = dynamic_ks.tolist()
        for gt_idx in range(num_gt):         
            # 根据每行分配的候选框数量num=dynamic_ks[gt_idx]，由cost找出前num最小cost的位置，
            # 作为分配的候选框
            _, pos_idx = torch.topk(
                cost[gt_idx], k=dynamic_ks[gt_idx], largest=False # 根据每一行（每一个gt）的cost矩阵 取出动态的每个gt最低的前k个候选框的index值
            )
        ...
        fg_mask_inboxes = matching_matrix.sum(0) > 0 # 每列求和并找出大于零的列
        num_fg = fg_mask_inboxes.sum().item()   # 总共的候选框个数
